...
# Define regex patterns for template shortcuts (v3)
# Note: DO NOT use \b near special symbols like § or (
patterns = [
    ...
    # 8. New legal sub-templates (no \b near § or parenthesis)
    ...
    # 9. Financial template patterns
    r'\bC\.\s+Finanzielle\s+Auswirkungen\b',
    r'\bKosten\s+werden\s+auf\s+.*?\s+geschätzt\b',
    r'\bMilliarden\s+Euro\s+geschätzt\b',
    
    # 10. Remaining legal sub-phrases
    r'\bAusübung\s+des\s+Ermessens\b',
    r'\bdes\s+Ermessens\b',
    r'\bwahrt\s+bei\b',
    r'\bgesetzlichen\s+Grenzen\b',
    r'\bnach\s+Prüfung\s+der\b',
    r'\bbei\s+Vorliegen\s+besonderer\b',
    r'\bvorliegen\s+besonderer\b'
]
...
print("\n--- Top 30 Bigram Differences After Expanded Regex Cleaning (v3) ---")
